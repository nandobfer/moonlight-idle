import { ColumnType, ItemTier } from "./class/Item/Equipment"
import { MonsterData } from "./types/monster_assets"

export const monsters: { [key: string]: MonsterData } = {
    [1]: {
        name: "rat",
        max_level: 3,
        attack_power: 3,
        attack_speed: 0.5,
        health: 85,
        gold_base: 3,
        spritesheet: require("../assets/game/monsters/rat/spritesheet.png"),
        drops: [
            { tier: ItemTier.wooden, column: ColumnType.sword, chance: 5 },
            { tier: ItemTier.wooden, column: ColumnType.shield, chance: 5 },
            { tier: ItemTier.wooden, column: ColumnType.trinket, chance: 5 },
        ],
    },

    [2]: {
        name: "bat",
        max_level: 5,
        attack_power: 5,
        attack_speed: 0.8,
        health: 70,
        gold_base: 10,
        spritesheet: require("../assets/game/monsters/bat/spritesheet.png"),
        drops: [
            { tier: ItemTier.wooden, column: ColumnType.sword, chance: 8 },
            { tier: ItemTier.wooden, column: ColumnType.shield, chance: 8 },
            { tier: ItemTier.wooden, column: ColumnType.trinket, chance: 8 },
            { tier: ItemTier.bronze, column: ColumnType.sword, chance: 3 },
            { tier: ItemTier.bronze, column: ColumnType.shield, chance: 3 },
            { tier: ItemTier.bronze, column: ColumnType.trinket, chance: 3 },
        ],
    },
    [3]: {
        name: "slime",
        max_level: 5,
        attack_power: 5,
        attack_speed: 0.4,
        gold_base: 50,
        health: 200,
        spritesheet: require("../assets/game/monsters/slime/spritesheet.png"),
        drops: [
            { tier: ItemTier.bronze, column: ColumnType.sword, chance: 5 },
            { tier: ItemTier.bronze, column: ColumnType.shield, chance: 5 },
            { tier: ItemTier.bronze, column: ColumnType.trinket, chance: 5 },
        ],
    },
    [4]: {
        name: "goblin",
        max_level: 10,
        attack_power: 12,
        attack_speed: 0.8,
        gold_base: 100,
        health: 140,
        spritesheet: require("../assets/game/monsters/goblin/spritesheet.png"),
        drops: [
            { tier: ItemTier.bronze, column: ColumnType.sword, chance: 8 },
            { tier: ItemTier.bronze, column: ColumnType.shield, chance: 8 },
            { tier: ItemTier.bronze, column: ColumnType.trinket, chance: 8 },
            { tier: ItemTier.iron, column: ColumnType.sword, chance: 3 },
            { tier: ItemTier.iron, column: ColumnType.shield, chance: 3 },
            { tier: ItemTier.iron, column: ColumnType.trinket, chance: 3 },
        ],
    },
    [5]: {
        name: "wolf",
        max_level: 15,
        attack_power: 30,
        attack_speed: 0.7,
        gold_base: 250,
        health: 300,
        spritesheet: require("../assets/game/monsters/wolf/spritesheet.png"),
        drops: [
            { tier: ItemTier.iron, column: ColumnType.sword, chance: 5 },
            { tier: ItemTier.iron, column: ColumnType.shield, chance: 5 },
            { tier: ItemTier.iron, column: ColumnType.trinket, chance: 5 },
        ],
    },
    [6]: {
        name: "skeleton",
        max_level: 25,
        attack_power: 40,
        attack_speed: 0.2,
        gold_base: 500,
        health: 1200,
        spritesheet: require("../assets/game/monsters/skeleton/spritesheet.png"),
        drops: [
            { tier: ItemTier.iron, column: ColumnType.sword, chance: 8 },
            { tier: ItemTier.iron, column: ColumnType.shield, chance: 8 },
            { tier: ItemTier.iron, column: ColumnType.trinket, chance: 8 },
            { tier: ItemTier.steel, column: ColumnType.sword, chance: 3 },
            { tier: ItemTier.steel, column: ColumnType.shield, chance: 3 },
            { tier: ItemTier.steel, column: ColumnType.trinket, chance: 3 },
        ],
    },
    [7]: {
        name: "werewolf",
        max_level: 50,
        attack_power: 50,
        attack_speed: 1,
        gold_base: 1500,
        health: 750,
        spritesheet: require("../assets/game/monsters/werewolf/spritesheet.png"),
        drops: [
            { tier: ItemTier.steel, column: ColumnType.sword, chance: 8 },
            { tier: ItemTier.steel, column: ColumnType.shield, chance: 8 },
            { tier: ItemTier.steel, column: ColumnType.trinket, chance: 8 },
            { tier: ItemTier.dark_iron, column: ColumnType.sword, chance: 3 },
            { tier: ItemTier.dark_iron, column: ColumnType.shield, chance: 3 },
            { tier: ItemTier.dark_iron, column: ColumnType.trinket, chance: 3 },
        ],
    },
}
